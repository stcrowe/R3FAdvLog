...
reverse R3F_LOG_classes_objects_transportables;
		...
		case 28:
		{
			...
			switch (KEY_mode) do
			{
				...
				case "c":
				{
					...
					R3F_LOG_classes_objects_transportables = R3F_LOG_classes_objects_transportables - [_class_name];
					R3F_LOG_classes_objects_transportables = R3F_LOG_classes_objects_transportables + [_class_name];

					R3F_LOG_CFG_objects_transportables_new = [];
					for [{_idx = 0}, {_idx < count R3F_LOG_CFG_can_be_transported_cargo}, {_idx = _idx+1}] do
					{
						...
						{
							R3F_LOG_CFG_objects_transportables_new = R3F_LOG_CFG_objects_transportables_new + [R3F_LOG_CFG_can_be_transported_cargo select _idx];
						};
					};
					R3F_LOG_CFG_can_be_transported_cargo = +R3F_LOG_CFG_objects_transportables_new;
...
while {BOOL_continue} do
{
	...
	while {_idx_current_vehicle == IDX_requested_vehicle} do
	{
		...
		_class_infos = _class_infos + format ["<t color='#00ff00'>[%1] Cargo-able (transport)</t><br/>", if (_class_name in R3F_LOG_classes_objects_transportables) then {format ["C%1", R3F_LOG_CFG_can_be_transported_cargo select (R3F_LOG_classes_objects_transportables find _class_name) select 1]} else {" "}];
		_class_infos = _class_infos + "<br/>";
		...
		for [{_j = 0}, {_j < count _tab_inheritance_tree}, {_j = _j+1}] do
		{
			...
			if (_class_name_inherit in R3F_LOG_classes_objects_transportables)
			then {_options = _options + format ["<t color='#00ff00'>C%1</t>", R3F_LOG_CFG_can_be_transported_cargo select (R3F_LOG_classes_objects_transportables find _class_name_inherit) select 1];};
