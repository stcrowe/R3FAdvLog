 ...
 * R3F_LOG_FNCT_3D_get_objects_genants_rayon
 * R3F_LOG_FNCT_3D_get_bounding_box_depuis_classname
 * R3F_LOG_FNCT_3D_get_hauteur_terrain_min_max_object
 *
 * // Transformation 3D
...
R3F_LOG_FNCT_3D_ray_intersect_bbox_obj =
{
	private ["_ray_pos", "_ray_dir", "_object"];
	...
	[
		_object worldToModel _ray_pos,
		// (_object worldToModel _ray_dir) vectorDiff (_object worldToModel [0,0,0]), Manque de précision numérique, d'où l'expression ci-dessous
		(_object worldToModel ASLtoATL (_ray_dir vectorAdd getPosASL _object)) vectorDiff (_object worldToModel ASLtoATL (getPosASL _object)),
		boundingBoxReal _object select 0,
		boundingBoxReal _object select 1
...
R3F_LOG_FNCT_3D_cam_intersect_bbox_obj =
{
	private ["_object", "_pos_cam", "_pos_devant", "_dir_cam"];

	_object = _this select 0;

	if (isNull _object) exitWith {1E39};
	...
	[_pos_cam, _dir_cam, _object] call R3F_LOG_FNCT_3D_ray_intersect_bbox_obj
};
...
R3F_LOG_FNCT_3D_intersect_bounding_sphere_objs =
{
	...
	//_pos1 = [0,0,0];
	//_rayon1 = (vectorMagnitude (boundingBoxReal _object1 select 0)) max (vectorMagnitude (boundingBoxReal _object1 select 1));
	//_pos2 = _object1 worldToModel (_object2 modelToWorld [0,0,0]);
	//_rayon2 = (vectorMagnitude (boundingBoxReal _object2 select 0)) max (vectorMagnitude (boundingBoxReal _object2 select 1));
	// Retour : (_pos1 distance _pos2) <= (_rayon1 + _rayon2)

	// Ce qui donne
	vectorMagnitude (_object1 worldToModel (_object2 modelToWorld [0,0,0])) <= (
		((vectorMagnitude (boundingBoxReal _object1 select 0)) max (vectorMagnitude (boundingBoxReal _object1 select 1)))
	+
		((vectorMagnitude (boundingBoxReal _object2 select 0)) max (vectorMagnitude (boundingBoxReal _object2 select 1)))
	)
};
...
R3F_LOG_FNCT_3D_bbox_intersect_bbox =
{
	private ["_object1", "_object2", "_bbox1_min", "_bbox1_max", "_bbox2_min", "_bbox2_max", "_intersect", "_coins", "_rayons"];

	_object1 = _this select 0;
	_bbox1_min = _this select 1;
	_bbox1_max = _this select 2;
	_object2 = _this select 3;
	...
	if !(
			[
				_object2 worldToModel (_object1 modelToWorld [0,0,0]),
		...
		&&
			[
				_object1 worldToModel (_object2 modelToWorld [0,0,0]),
	...
	// Composition des coordonnées des 8 coins de la bounding box de l'objet1, dans l'espace du modèle _object2
	_coins set [0, _object2 worldToModel (_object1 modelToWorld [_bbox1_min select 0, _bbox1_min select 1, _bbox1_min select 2])];
	_coins set [1, _object2 worldToModel (_object1 modelToWorld [_bbox1_min select 0, _bbox1_min select 1, _bbox1_max select 2])];
	_coins set [2, _object2 worldToModel (_object1 modelToWorld [_bbox1_min select 0, _bbox1_max select 1, _bbox1_min select 2])];
	_coins set [3, _object2 worldToModel (_object1 modelToWorld [_bbox1_min select 0, _bbox1_max select 1, _bbox1_max select 2])];
	_coins set [4, _object2 worldToModel (_object1 modelToWorld [_bbox1_max select 0, _bbox1_min select 1, _bbox1_min select 2])];
	_coins set [5, _object2 worldToModel (_object1 modelToWorld [_bbox1_max select 0, _bbox1_min select 1, _bbox1_max select 2])];
	_coins set [6, _object2 worldToModel (_object1 modelToWorld [_bbox1_max select 0, _bbox1_max select 1, _bbox1_min select 2])];
	_coins set [7, _object2 worldToModel (_object1 modelToWorld [_bbox1_max select 0, _bbox1_max select 1, _bbox1_max select 2])];
	...
	// Composition des coordonnées des 8 coins de la bounding box de l'objet2, dans l'espace du modèle _object1
	_coins set [0, _object1 worldToModel (_object2 modelToWorld [_bbox2_min select 0, _bbox2_min select 1, _bbox2_min select 2])];
	_coins set [1, _object1 worldToModel (_object2 modelToWorld [_bbox2_min select 0, _bbox2_min select 1, _bbox2_max select 2])];
	_coins set [2, _object1 worldToModel (_object2 modelToWorld [_bbox2_min select 0, _bbox2_max select 1, _bbox2_min select 2])];
	_coins set [3, _object1 worldToModel (_object2 modelToWorld [_bbox2_min select 0, _bbox2_max select 1, _bbox2_max select 2])];
	_coins set [4, _object1 worldToModel (_object2 modelToWorld [_bbox2_max select 0, _bbox2_min select 1, _bbox2_min select 2])];
	_coins set [5, _object1 worldToModel (_object2 modelToWorld [_bbox2_max select 0, _bbox2_min select 1, _bbox2_max select 2])];
	_coins set [6, _object1 worldToModel (_object2 modelToWorld [_bbox2_max select 0, _bbox2_max select 1, _bbox2_min select 2])];
	_coins set [7, _object1 worldToModel (_object2 modelToWorld [_bbox2_max select 0, _bbox2_max select 1, _bbox2_max select 2])];
	...
	// Composition des 12 rayons [pos, dir, longueur] correspondant aux segments de la bounding box de l'objet2, dans l'espace du modèle _object1
	_rayons = [];
...
R3F_LOG_FNCT_3D_bbox_intersect_bbox_objs =
{
	...
	[
		_object1,
		boundingBoxReal _object1 select 0,
		boundingBoxReal _object1 select 1,
		_object2,
		boundingBoxReal _object2 select 0,
		boundingBoxReal _object2 select 1
...
R3F_LOG_FNCT_3D_mesh_collision_objs =
{
	private ["_object1", "_object2", "_object_test1", "_object_test2", "_force_test_mesh", "_pos_test", "_num_frame_start", "_collision"];
	...
	systemChat format ["PROBABLE INTERSECT MESH : %1 @ %2", _object2, time];//TODO REMOVE

	_pos_test = ATLtoASL (player modelToWorld [0,16,20]);// TODO remplacer par R3F_LOG_FNCT_3D_tirer_position_degagee_ciel

	_object_test1 = (typeOf _object1) createVehicleLocal ([] call R3F_LOG_FNCT_3D_tirer_position_degagee_ciel);
	_object_test1 setVectorDirAndUp [vectorDir _object1, vectorUp _object1];
	_object_test1 allowDamage false;
	_object_test1 addEventHandler ["EpeContactStart", {if (!isNull (_this select 1)) then {(_this select 0) setVariable ["R3F_LOG_3D_collision", true, false];};}];
	_object_test1 setVariable ["R3F_LOG_3D_collision", false, false];

	_object_test2 = (typeOf _object2) createVehicleLocal ([] call R3F_LOG_FNCT_3D_tirer_position_degagee_ciel);
	_object_test2 setVectorDirAndUp [vectorDir _object2, vectorUp _object2];
	_object_test2 allowDamage false;
	_object_test2 addEventHandler ["EpeContactStart", {if (!isNull (_this select 1)) then {(_this select 0) setVariable ["R3F_LOG_3D_collision", true, false];};}];
	_object_test2 setVariable ["R3F_LOG_3D_collision", false, false];

	_object_test1 setVelocity [0,0,0];
	_object_test1 setVectorDirAndUp [vectorDir _object1, vectorUp _object1];
	_object_test1 setPosASL _pos_test;
	_object_test2 setVelocity [0,0,0];
	_object_test2 setVectorDirAndUp [vectorDir _object2, vectorUp _object2];
	_object_test2 setPosASL (_pos_test vectorAdd ((_object1 worldToModel (_object2 modelToWorld [0,0,0])) vectorDiff (_object1 modelToWorld [0,0,0])));

	_num_frame_start = diag_frameno;
	waitUntil
	{
		_collision = (_object_test1 getVariable "R3F_LOG_3D_collision") || (_object_test2 getVariable "R3F_LOG_3D_collision");
		_collision || (diag_frameno - _num_frame_start > 10)
	};

	if (_collision) then {systemChat format ["RESULTAT COLLISION: %1 @ %2", _object2, time];};//TODO REMOVE

	sleep 0.02;// TODO REMOVE

	deleteVehicle _object_test1;
	deleteVehicle _object_test2;
...
R3F_LOG_FNCT_3D_tirer_position_degagee_sol =
{
	...
	private ["_nb_tirages", "_objects_genants", "_position_degagee", "_rayon_curr", "_angle_curr", "_intersect"];
	...
	for [
		...
		},
		{
			...
			else
			{
				...
				} forEach ([_position_degagee, _rayon_degage+15] call R3F_LOG_FNCT_3D_get_objects_genants_rayon);
...
R3F_LOG_FNCT_3D_cursorTarget_distance_bbox =
{
	...
	if (!isNull _object && !isNull _player && alive _player && cameraOn == _player) then
	{
		[
			_object,
			[
				_object worldToModel (_player modelToWorld (_player selectionPosition "head")),
				boundingBoxReal _object select 0,
				boundingBoxReal _object select 1
...
/**
 * Returns the object pointed to by the player to a max distance of the bounding box of the object pointed
...
R3F_LOG_FNCT_3D_cursorTarget_virtuel =
{
	private ["_liste_ingores", "_distance_max", "_player", "_object_pointe", "_cursorTarget_distance"];
	...
	{
		_object_pointe = cursorTarget;
	}
	else
	{
		private ["_vec_dir_unite_world", "_pos_unite_world", "_liste_objects"];

		_vec_dir_unite_world = (ATLtoASL positionCameraToWorld [0, 0, 1]) vectorDiff (ATLtoASL positionCameraToWorld [0,0,0]);
		_pos_unite_world = _player modelToWorld (_player selectionPosition "head");

		_liste_objects = lineIntersectsObjs [
...

		{
			...
			) exitWith {_object_pointe = _x;};
		} forEach _liste_objects;
	};
...
R3F_LOG_FNCT_3D_get_huit_coins_bounding_box_world =
{
	private ["_object", "_bbox_min", "_bbox_max"];

	_object = _this select 0;

	_bbox_min = boundingBoxReal _object select 0;
	_bbox_max = boundingBoxReal _object select 1;

	[
		_object modelToWorld [_bbox_min select 0, _bbox_min select 1, _bbox_min select 2],
		_object modelToWorld [_bbox_min select 0, _bbox_min select 1, _bbox_max select 2],
		_object modelToWorld [_bbox_min select 0, _bbox_max select 1, _bbox_min select 2],
		_object modelToWorld [_bbox_min select 0, _bbox_max select 1, _bbox_max select 2],
		_object modelToWorld [_bbox_max select 0, _bbox_min select 1, _bbox_min select 2],
		_object modelToWorld [_bbox_max select 0, _bbox_min select 1, _bbox_max select 2],
		_object modelToWorld [_bbox_max select 0, _bbox_max select 1, _bbox_min select 2],
		_object modelToWorld [_bbox_max select 0, _bbox_max select 1, _bbox_max select 2]
	]
};
...
R3F_LOG_FNCT_3D_get_bounding_box_depuis_classname =
{
	private ["_classe", "_object_tmp", "_bbox"];

	_classe = _this select 0;

	// Création du véhicule local temporaire dans le ciel pour connaître la bounding box de l'objet
	_object_tmp = _classe createVehicleLocal ([] call R3F_LOG_FNCT_3D_tirer_position_degagee_ciel);
	sleep 0.01;
	_bbox = boundingBoxReal _object_tmp;
	deleteVehicle _object_tmp;
...
R3F_LOG_FNCT_3D_get_hauteur_terrain_min_max_object =
{
	private ["_object", "_x1", "_x2", "_y1", "_y2", "_z", "_hauteur_min", "_hauteur_max", "_hauteur"];

	_object = _this select 0;

	_x1 = boundingBoxReal _object select 0 select 0;
	_x2 = boundingBoxReal _object select 1 select 0;
	_y1 = boundingBoxReal _object select 0 select 1;
	_y2 = boundingBoxReal _object select 1 select 1;

	_z = boundingBoxReal _object select 0 select 2;
	...
	{
		_hauteur = getTerrainHeightASL (_object modelToWorld _x);
...
R3F_LOG_FNCT_3D_tracer_bbox =
{
	private ["_object", "_bbox_min", "_bbox_max", "_coins", "_couleur"];
	...
	if !(isNull _object) then
	{
		...
		_coins = [_object] call R3F_LOG_FNCT_3D_get_huit_coins_bounding_box_world;
...
R3F_LOG_FNCT_3D_tracer_bbox_obj =
{
	...
	if !(isNull _object) then
	{
		[_object, boundingBoxReal _object select 0, boundingBoxReal _object select 1] call R3F_LOG_FNCT_3D_tracer_bbox;
	};
};
...
if (isNil "R3F_LOG_player_moves_object") then {R3F_LOG_player_moves_object = objNull};
addMissionEventHandler ["Draw3D",
{
	if !(isNull player) then
	{
		...
		if (!isNull R3F_LOG_player_moves_object) then
		{
			//[R3F_LOG_player_moves_object] call R3F_LOG_FNCT_3D_tracer_bbox_obj;

			{
				if ([
					R3F_LOG_player_moves_object,
					boundingBoxReal R3F_LOG_player_moves_object select 0,
					boundingBoxReal R3F_LOG_player_moves_object select 1,
					_x,
					boundingBoxReal _x select 0,
					boundingBoxReal _x select 1
			...
			} forEach (nearestObjects [R3F_LOG_player_moves_object, ["All"], 15] - [player, R3F_LOG_player_moves_object]);
		}
		else
		{
			if (false && !isNull _object) then
			{
				hintSilent format ["%1 | %2 | %3", typeOf _object, [_object] call R3F_LOG_FNCT_3D_cam_intersect_bbox_obj,
					[_object worldToModel (positionCameraToWorld [0, 0, 0]), boundingBoxReal _object select 0, boundingBoxReal _object select 1] call R3F_LOG_FNCT_3D_pos_est_dans_bbox];

				[_object] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
			}
			else
			{
				_cursorTarget_distance = call R3F_LOG_FNCT_3D_cursorTarget_distance_bbox;
		...
		//if !(isNil "R3F_LOG_liste_objects_en_deplacement") then
		//{
		//	{
		//		[_x] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
		//	} forEach R3F_LOG_liste_objects_en_deplacement;
		//};

		//{
		//	[_x] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
		//} forEach [[[R3F_LOG_player_moves_object]] call R3F_LOG_FNCT_3D_cursorTarget_virtuel];

		//{
		//	[_x] call R3F_LOG_FNCT_3D_tracer_bbox_obj;
		//} forEach ([player, 100] call R3F_LOG_FNCT_3D_get_objects_genants_rayon);
	};
}];
#endif